...
%The differential is approximated with a backward difference.
clear all;
...
stepMat = toeplitz([(1 + 2*mu - beta) -(mu), zeros(1,length(x)-4)],...
             [(1 + 2*mu - beta) (-mu + beta), zeros(1,length(x)-4)]);

for n = 1:1:(length(t)-1)
    
    %compute the values for the next time step.
    U(n+1,2:(end-1)) = stepMat\U(n,2:(end-1))'...
    + [(mu + beta)*U(n+1,1); zeros(length(x)-4,1); (mu)*U(n+1,end)];
